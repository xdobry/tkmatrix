...
public:
	...
	matrix(matrix &);
	matrix & operator = (matrix &); // mein Compiler wollte das nicht schlucken
	void load(matrix &); // Ersatz für Obengennanten
	~matrix();
	...
	int sstellencount(sets &);	// gibt die zahl der sprungstellen zurück
